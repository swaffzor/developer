<?php
	...
	//date("Y-m-d", strtotime("-1 days", strtotime($ddate)))
	$sunday = getSundayDate($date);
	for ($i=0; $i<7; $i++){
		$test = (string)(date("Y-m-d", strtotime("+".$i." days", strtotime($sunday))));
		$query = mysqli_query($con, "SELECT * FROM Data WHERE Date = '". $test ."' AND Name = '".$_COOKIE["name"]."'");
...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			...
			function onButtonPress(sender){
				//alert(sender.id);
				document.getElementById("chbxMonday").checked = true;
				showHide("divMonday", "chbxMonday") ;
			}
	...
	<table cellspacing="15px"><tr><td valign="top" width="500px">
		...
		<td valign="top">
			...
			<div name="recap_summary" id="recap_summary">
				
			</div>
			<div name="divMonday" class="hide">
				<input type="checkbox" name="chbxMonday" id="chbxMonday" class="hide">
				<? echo $day[1]; ?>
			</div>
			<?
